...
set_default_v(void * output, int ctype, pbc_var defv) {
	...
	case CTYPE_FLOAT:
		*(float *)output = (float)defv->real;
		break;
	case CTYPE_BOOL:
		*(__crt_bool *)output = (defv->integer.low != 0);
		break;
	case CTYPE_INT8:
...
_pack_number(int ptype , int ctype , struct pbc_slice *s, void *input) {
	...
	if (ctype == CTYPE_VAR) {
		...
		case CTYPE_INT8:
			...
			break;
		case CTYPE_BOOL:
			var->integer.low = *(__crt_bool *)input;
			var->integer.hi = 0;
			break;
		case CTYPE_DOUBLE:
...
_is_default(struct _pattern_field * pf, void * in) {
	...
	case CTYPE_BOOL:
		if (pf->defv->integer.low)
			return *(__crt_bool *)in == 1;
		else
			return *(__crt_bool *)in == 0;
	}
...
pbc_pattern_unpack(struct pbc_pattern *pat, struct pbc_slice *s, void * output) {
	...
	__crt_bool * field = (__crt_bool *)alloca(pat->count * sizeof(__crt_bool));
	memset(field, 0, pat->count * sizeof(__crt_bool));

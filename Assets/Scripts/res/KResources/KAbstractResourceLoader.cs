...
namespace slGame.KResources
{
    ...
    public abstract class AbstractResourceLoader : IAsyncObject
    {
            ...
            params object[] initArgs) where T : AbstractResourceLoader, new()
        {
            ...
            {
                Debug.LogError(string.Format("{0}:AutoNew]url为空", typeof(T)));
            }
        ...
        protected virtual void OnFinish(object resultObj)
        {
            Action doFinish = () =>
            {
                ...
                {
                    //Dispose();
                    Debug.Log( string.Format("BaseResourceLoader:OnFinish]时，准备Disposed {0}", Url));
                }
            };
            ...
            if (callback != null)
            {
                if (IsCompleted)
                {
                    if (ResultObject == null)
                        Debug.LogWarning(string.Format("Null ResultAsset {0}", Url));
        ...
        public virtual void Release()
        {
            ...
            {
                Debug.LogWarning(string.Format("[{0}]Too many dipose! {1}, Count: {2}", GetType().Name, this.Url, RefCount));
            }
            ...
            if (RefCount <= 0)
            {
                ...
                if (Debug.isDebugBuild)
                {
                    if (RefCount < 0)
                    {
                        Debug.LogError(string.Format("[{3}]RefCount< 0, {0} : {1}, NowRefCount: {2}, Will be fix to 0", GetType().Name,
                    ...
                    {
                        Debug.LogError(string.Format("[{1}]UnUsesLoader exists: {0}", this, GetType()));
                    }
                }
        ...
        private void Dispose()
        {
            ...
            if (!IsForceNew)
            {
                ...
                //if (Url != null) // TODO: 以后去掉
                {
                    ...
                    if (!bRemove)
                    {
                        Debug.LogWarning(string.Format("[{0}:Dispose]No Url: {1}, Cur RefCount: {2}", type.Name, Url, RefCount));
                    }
                }
            }
        ...
        public virtual void ForceDispose()
        {
            ...
            if (RefCount != 1)
            {
                Debug.LogWarning(string.Format("[ForceDisose]Use force dispose to dispose loader, recommend this loader RefCount == 1"));
            }
            Dispose();
        }

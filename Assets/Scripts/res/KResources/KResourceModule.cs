...
namespace slGame.KResources
{
    ...
    public class KResourceModule : MonoBehaviour
    {
        ...
        {
            return string.Format(path + ".bytes", formats);
        }
             ...
             bool isLog = true)
        {
            ...
            if (ResourcePathPriorityType == KResourcePathPriorityType.PersistentDataPathPriority) // 優先下載資源模式
            {
                if (hasDocUrl)
                {
                    if (Application.isEditor)
                        Debug.LogWarning(string.Format("[Use PersistentDataPath] {0}", docUrl));
                    fullPath = docUrl;
            ...
            {
                if (isLog)
                    Debug.LogError(string.Format("[Not Found] StreamingAssetsPath Url Resource: {0}", url));
                fullPath = null;
        ...
        public static bool TryGetInAppStreamingUrl(string url, bool withFileProtocol, out string newUrl)
        {
            ...
            if (Application.isEditor)
            {
                ...
                if (!result)
                {
                    Debug.LogError(string.Format("[大小写敏感]发现一个资源 {0}，大小写出现问题，在Windows可以读取，手机不行，请改表修改！", url));
                }
            }
            return true;
        }
        ...
        private void Awake()
        {
            ...
            {
                Debug.Log(string.Format("ResourceManager ApplicationPath: {0}", ApplicationPath));
                Debug.Log(string.Format("ResourceManager ProductPathWithProtocol: {0}", ProductPathWithProtocol));
                Debug.Log(string.Format("ResourceManager ProductPathWithoutProtocol: {0}", ProductPathWithoutFileProtocol));
                Debug.Log(string.Format("ResourceManager DocumentResourcesPath: {0}", DocumentResourcesPath));
        ...
        public static string EditorAssetBundleFullPath
        {
            get
            {
                string editorAssetBundlePath = Path.GetFullPath("Product/Bundles"); // for editoronly
        ...
        public static void LogRequest(string resType, string resPath)
        {
            ...
            Debug.Log(string.Format("[Request] {0}, {1}", resType, resPath));
        }
        ...
        public static void LogLoadTime(string resType, string resPath, System.DateTime begin)
        {
            ...
            Debug.Log(string.Format("[Load] {0}, {1}, {2}s", resType, resPath, (System.DateTime.Now - begin).TotalSeconds));
        }

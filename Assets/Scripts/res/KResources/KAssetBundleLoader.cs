    ...
    public class AssetBundleLoader : AbstractResourceLoader
    {
        ...
        private IEnumerator LoadAssetBundle(string relativeUrl)
        {
            ...
            if (!bytesLoader.IsSuccess)
            {
                ...
                Debug.LogError(string.Format("[AssetBundleLoader]Error Load Bytes AssetBundle: {0}", relativeUrl));
                OnFinish(null);
                yield break;
            }
            ...
            if (assetBundle == null)
                Debug.LogError(string.Format("WWW.assetBundle is NULL: {0}", RelativeResourceUrl));
        ...
        public override void Release()
        {
            if (Application.isEditor)
            {
                ...
                {
                    Debug.LogError(string.Format("要释放Arial字体！！错啦！！builtinextra:{0}", Url));
